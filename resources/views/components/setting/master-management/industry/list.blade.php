            ...
            <tbody class="bg-white">
                @foreach($industries as $industry)
                    <tr class="text-left cursor-default whitespace-nowrap">
                        ...
                        <td class="py-1 px-2 border">{{ $industry->industry_name }}</td>
                        <td class="py-1 px-2 border text-right">{{ number_format($industry->clients_count) }}</td>
                        <td class="py-1 px-2 border text-right">{{ number_format($industry->sort_order) }}</td>
                        <td class="py-1 px-2 border text-center">
                            <div class="flex items-center justify-center gap-2">
                                <span class="whitespace-nowrap text-xs">
                                    {{ CarbonImmutable::parse($industry->updated_at)->isoFormat('Y年MM月DD日(ddd) HH:mm:ss').' ('.CarbonImmutable::parse($industry->updated_at)->diffForHumans().')' }}
                                </dpan>
                            </div>
                        </td>
                    </tr>
